...
| `POST /getStationList` | plant inventory | `pageNo`/`pageSize=100`; BOTH documented variants parsed on this path (direct list and `{list,pageNo,pageSize,pageCount,total}`); the paginated envelope is validated strictly (all four fields mandatory, stable across pages, final count == `total`) and a failed validation keeps the previous inventory; pages per refresh are bounded by the daily budget; refresh spacing is derived from the budget slots consumed — one per request, the failCode 305 retry included — never every KPI cycle |
